...
Explosion::Explosion(GameObject* parent)
  ...
  // float current_time = scene_->game_time().current_time();
  // for (int i = 0; i < 250; ++i) {
  //   particles_.at(i) = ExplosionParticle(transform().pos(), current_time);
  // }
}
...
Particle FireParticle(glm::vec3 startpos, float current_time) {
  ...
  p.accel = 0.5f*normalize(startpos + glm::vec3{0, 4, 0} - p.pos + 0.2f*RandomDir());
  p.speed = 0.2f*RandomDir() + 4.0f*p.accel;

  // 1 - 3 sec lifespan
  p.lifespan = (rand()%20) / 10.0 + 1;

  p.scale = 0.025 + 0.025*Rand01();
  return p;
}
...
Particle ExplosionParticle(glm::vec3 startpos, float current_time) {
  Particle p;
  p.born_at = current_time;
  p.pos = startpos + RandomDir();
  p.accel = (5.0f + 5.0f*Rand01())*normalize(RandomDir());
